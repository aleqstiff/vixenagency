...
export default function MegaNav({ locale, posts = [] }: {
  ...
  const PILL: React.CSSProperties = {
    ...
    color:"rgba(26,22,18,0.65)", display:"flex", alignItems:"center", gap:5,
  };
  ...
  return (
    <>
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"rgba(250,248,245,0.96)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(26,22,18,0.08)" }}>
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 20px", height:64, display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
            ...
            <div style={{ position:"relative" }} onMouseEnter={() => setDesktop("sv")}>
              ...
              {desktop === "sv" && (
                <div style={{ ...MEGA, minWidth:600 }}>
                  <p style={{ fontSize:10, fontWeight:700, color:"var(--rose)", textTransform:"uppercase", letterSpacing:"0.12em", marginBottom:16 }}>{t(locale,"nav_mega_services_title")}</p>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    {services.map(s => (
                      <Link key={s.slug} href={`/${locale}/servicios/${s.slug}/`}
                        style={{ display:"flex", gap:12, padding:"10px 14px", borderRadius:12, background:"var(--cream)", border:"1px solid var(--border)", textDecoration:"none", transition:"border-color .15s" }}
                        onMouseEnter={e => ((e.currentTarget as HTMLElement).style.borderColor="rgba(200,112,90,0.4)")}
                        onMouseLeave={e => ((e.currentTarget as HTMLElement).style.borderColor="var(--border)")}>
                        <div style={{ width:36, height:36, borderRadius:10, background:"rgba(200,112,90,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>
                          {s.icon}
                        </div>
                        <div>
              ...
              <div style={{ position:"relative" }} onMouseEnter={() => setDesktop("co")}>
                ...
                {desktop === "co" && (
                  <div style={{ ...MEGA, minWidth:340 }}>
                    <p style={{ fontSize:10, fontWeight:700, color:"var(--rose)", textTransform:"uppercase", letterSpacing:"0.12em", marginBottom:14 }}>{t(locale,"nav_mega_countries_title")}</p>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
                      {countries.map(ck => {
                        const c = COUNTRIES[ck];
                        return (
                          <Link key={ck} href={`/${locale}/${ck}/`}
                            style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:10, background:"var(--cream)", border:"1px solid var(--border)", textDecoration:"none" }}
                            onMouseEnter={e => ((e.currentTarget as HTMLElement).style.borderColor="rgba(200,112,90,0.4)")}
                            onMouseLeave={e => ((e.currentTarget as HTMLElement).style.borderColor="var(--border)")}>
            ...
            <div style={{ position:"relative" }} onMouseEnter={() => setDesktop("blog")}>
              ...
              {desktop === "blog" && (
                <div style={{ ...MEGA, minWidth:380 }}>
                  <p style={{ fontSize:10, fontWeight:700, color:"var(--rose)", textTransform:"uppercase", letterSpacing:"0.12em", marginBottom:14 }}>
                  ...
                  <div style={{ display:"grid", gap:6, marginBottom:12 }}>
                    {posts.slice(0,4).map(p => (
                      <Link key={p.slug} href={`/${locale}/blog/${p.slug}/`}
                        style={{ display:"flex", gap:10, padding:"10px 12px", borderRadius:10, background:"var(--cream)", border:"1px solid var(--border)", textDecoration:"none" }}
                        onMouseEnter={e => ((e.currentTarget as HTMLElement).style.borderColor="rgba(200,112,90,0.4)")}
                        onMouseLeave={e => ((e.currentTarget as HTMLElement).style.borderColor="var(--border)")}>
                        <div style={{ width:30, height:30, borderRadius:8, background:"rgba(200,112,90,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, flexShrink:0 }}>✍</div>
                        <p style={{ fontWeight:600, color:"var(--dark)", fontSize:13, lineHeight:1.3 }}>{p.title}</p>
                      </Link>
                    ))}
                  </div>
                  <Link href={`/${locale}/blog/`}
                    style={{ display:"block", textAlign:"center", padding:"10px", borderRadius:10, border:"1px solid var(--border2)", color:"var(--rose)", fontSize:13, fontWeight:700, textDecoration:"none" }}>
              ...
              {LOCALES.map(loc => (
                <Link key={loc} href={`/${loc}/`}
                  style={{ fontSize:10, padding:"3px 6px", borderRadius:5, textDecoration:"none",
                    ...
                    background: loc===locale ? "rgba(200,112,90,0.08)" : "transparent",
                    border:`1px solid ${loc===locale ? "rgba(200,112,90,0.3)" : "transparent"}` }}>
                  {loc}
                </Link>
              ))}
            </div>
          ...
          <button className="show-mobile" onClick={() => setMobile(!mobile)} aria-label="Menu"
            style={{ background:"rgba(26,22,18,0.06)", border:"1px solid var(--border)", borderRadius:10, padding:"10px 12px", cursor:"pointer", flexDirection:"column", gap:5, alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            {[0,1,2].map(i => (
              <span key={i} style={{ display:"block", width:22, height:2, background:"var(--dark)", borderRadius:1, transition:"all .25s",
      ...
      <div style={{ position:"fixed", top:64, left:0, right:0, bottom:0,
        background:"var(--cream)", zIndex:99, overflowY:"auto", padding:"20px 20px 40px",
        ...
        <div style={{ marginBottom:6 }}>
          <button onClick={() => setMSection(mSection==="sv"?null:"sv")}
            style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 16px", borderRadius:12, background:"#fff", border:"1px solid var(--border)", cursor:"pointer", color:"var(--dark)", fontWeight:700, fontSize:15 }}>
            <span>{L.sv[locale]}</span>
            <span style={{ color:"var(--rose)", fontSize:22, fontWeight:300, transform:mSection==="sv"?"rotate(45deg)":"none", transition:"transform .2s", lineHeight:1 }}>+</span>
          </button>
          {mSection === "sv" && (
            <div style={{ padding:"10px 0 0 6px", display:"grid", gap:6 }}>
              {services.map(s => (
                <Link key={s.slug} href={`/${locale}/servicios/${s.slug}/`} onClick={() => setMobile(false)}
                  style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", borderRadius:10, background:"#fff", border:"1px solid var(--border)", textDecoration:"none" }}>
        ...
        {countries.length > 1 && (
          <div style={{ marginBottom:6 }}>
            <button onClick={() => setMSection(mSection==="co"?null:"co")}
              ...
              <span style={{ color:"var(--rose)", fontSize:22, fontWeight:300, transform:mSection==="co"?"rotate(45deg)":"none", transition:"transform .2s", lineHeight:1 }}>+</span>
            </button>
            {mSection === "co" && (
                ...
                {countries.map(ck => {
                  ...
                  return (
                    <Link key={ck} href={`/${locale}/${ck}/`} onClick={() => setMobile(false)}
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 12px", borderRadius:10, background:"#fff", border:"1px solid var(--border)", textDecoration:"none" }}>
                      <span style={{ fontSize:20 }}>{c.flag}</span>
                      <div>
                        <p style={{ fontWeight:700, color:"var(--dark)", fontSize:13 }}>{c.name}</p>
                        <p style={{ color:"var(--muted)", fontSize:11 }}>{c.city}</p>
        ...
        <div style={{ marginBottom:6 }}>
          <button onClick={() => setMSection(mSection==="blog"?null:"blog")}
            style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 16px", borderRadius:12, background:"#fff", border:"1px solid var(--border)", cursor:"pointer", color:"var(--dark)", fontWeight:700, fontSize:15 }}>
            <span>Blog</span>
            <span style={{ color:"var(--rose)", fontSize:22, fontWeight:300, transform:mSection==="blog"?"rotate(45deg)":"none", transition:"transform .2s", lineHeight:1 }}>+</span>
          </button>
          {mSection === "blog" && (
            <div style={{ padding:"10px 0 0 6px", display:"grid", gap:6 }}>
              {posts.slice(0,5).map(p => (
                <Link key={p.slug} href={`/${locale}/blog/${p.slug}/`} onClick={() => setMobile(false)}
                  style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 14px", borderRadius:10, background:"#fff", border:"1px solid var(--border)", textDecoration:"none" }}>
                  <div style={{ width:30, height:30, borderRadius:8, background:"rgba(200,112,90,0.08)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, flexShrink:0 }}>✍</div>
                  <p style={{ fontWeight:600, color:"var(--dark)", fontSize:13, lineHeight:1.3 }}>{p.title}</p>
                </Link>
              ))}
        ...
        <Link href={`/${locale}/#como`} onClick={() => setMobile(false)}
          style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 16px", borderRadius:12, background:"#fff", border:"1px solid var(--border)", textDecoration:"none", marginBottom:6 }}>
            ...
            {LOCALES.map(loc => (
              <Link key={loc} href={`/${loc}/`} onClick={() => setMobile(false)}
                style={{ padding:"8px 16px", borderRadius:10, textDecoration:"none", fontFamily:"monospace", textTransform:"uppercase", fontWeight:800, fontSize:13,
                  color:loc===locale?"var(--rose)":"var(--muted2)",
                  background:loc===locale?"rgba(200,112,90,0.08)":"rgba(26,22,18,0.04)",
                  border:`1px solid ${loc===locale?"rgba(200,112,90,0.3)":"var(--border)"}` }}>
                {loc}
              </Link>
            ))}
          </div>
